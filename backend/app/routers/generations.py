...
async def generate_image(
    ...
    try:
        ...
        try:
            ...
            logger.exception(
                "vault read raised generation_id=%s brand_id=%s",
                generation_id, brand_id,
            )
            _mark_failed(
                generation_id, "VAULT_ERROR", "Failed to retrieve key from vault"
            )
            raise _error_response(
        ...
        if not api_key:
            _mark_failed(generation_id, "VAULT_ERROR", "Failed to retrieve key from vault")
            raise _error_response(
                502, "VAULT_ERROR", "Failed to retrieve key from vault"
            )
        ...
        # Intentional 4xx/5xx raised inside the pipeline (e.g. VAULT_ERROR) —
        # row is already marked failed by the inner handler; let it bubble.
        raise
    except Exception:
        logger.exception(
